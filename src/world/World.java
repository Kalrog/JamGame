...
public class World
{
    ...
    public World(Player player, int size)
    {
        ...
        {
            int random = (int) (Math.random() * (sumOfEncounterChance(encounters) + NO_ENCOUNTER_CHANCE) + 1) - NO_ENCOUNTER_CHANCE;
            if (random > 0)
                worldEncounters.add(encounterFactory(random, i));
        }
    ...
    private void removeCooldownEvents(Encounter encounter)
    {
        ...
        {
            if (encounter.distance <= encounter1.distance && encounter.distance + encounter.cooldown >= encounter1.distance && encounter1.priority <= encounter.priority)
            {
